...
# COPY 源路径 目标路径
COPY . $GOPATH/src/github.com/recallfuture/blog-api

# RUN 执行 go build .
RUN cd $GOPATH/src/github.com/recallfuture/blog-api && go build .
...
COPY --from=builder /opt/repo/src/github.com/recallfuture/blog-api /opt
